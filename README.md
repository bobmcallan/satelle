...
| **Planner transport** (`make planner-bench`) | Opt-in, repeatable Claude command vs Grok ACP planner comparison under `tests/plannerbench/` (`-tags plannerbench`). **Costs tokens**, never CI. Records wall time, token cost, plan-artifact correctness, read-only policy fidelity, and failure observability as JSON + Markdown evidence. Set `SATELLE_PLANNER_BENCH_RUNS` to repeat each fixture (default 3). |
