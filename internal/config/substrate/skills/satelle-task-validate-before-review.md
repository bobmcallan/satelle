...
description: Entry gate for a task EXECUTION (backlog → in_progress). An isolated reviewer judges that the run about to start is a well-formed, re-runnable execution of a VALID task — the task header it names exists and declares an ACTION and how success is VERIFIED. Reviewers-only (judges, never enacts). The embedded default named by the seeded task-execution workflow; a repo may override it under .satelle/skills. Pushes back when a run is not ready to begin.
---
