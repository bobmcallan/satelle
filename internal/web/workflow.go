...
// parseWorkflowDOT parses a fenced ```dot block in a workflow body into a wfSpec
// (node-centric: each DOT node is a step/state, each edge a transition). A
// reviewer node carries its gate as prompt="@skill:NAME"; the transition INTO a
// reviewer node is labelled with that skill. Returns ok=false when the body has
// no dot block, so parseWorkflow falls back to the inline-YAML grammar.
func parseWorkflowDOT(body string) (wfSpec, bool) {
	...
	for _, stmt := range dotStatements(block) {
		...
		if strings.Contains(t, "->") {
			...
			for i := 0; i+1 < len(ids); i++ {
				spec.Transitions = append(spec.Transitions, wfTransition{From: ids[i], To: ids[i+1]})
			}
			continue
		}
	...
	for _, name := range order {
		spec.States = append(spec.States, wfState{Name: name, Actor: nodes[name].actor})
	}
