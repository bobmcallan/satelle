...
digraph satelle_substrate_workflow {
  ...
  in_progress [agent=executor, prompt="@skill:substrate"]                                          // in-loop: author (and commit + push where substrate is tracked)
...
guardrails:
  always:
    ...
    - Where the repo tracks substrate in git: commit + push the slice IN-LOOP while engaged at in_progress (the commit gate refuses a commit with no engaged story). Repos that git-ignore .satelle/ close via the change-set channels without a commit.
