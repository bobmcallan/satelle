...
digraph satelle_task_workflow {
  ...
  // gate (wfdot: attrs["agent"] != ""). Default reviewer only — never a non-default
  // binding (satelle-dot-standard coded-check edge exception). after-review is LLM.
...
are authored in this repo's `.satelle/skills`, so there is no dangling
