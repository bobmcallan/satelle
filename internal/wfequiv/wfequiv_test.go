...
func TestBuildOrdersByPrerequisite(t *testing.T) {
	// Declared out of order on purpose: order must come from Requires/Provides.
	l := List{
		Category: "test",
		Steps: []Step{
			{Name: "release", Agent: "executor", Provides: "released", Requires: []string{"tested"}},
			{Name: "backlog", Start: true, Provides: "raised"},
			{Name: "code", Agent: "executor", Provides: "coded", Requires: []string{"raised"}},
			{Name: "test", Agent: "executor", Provides: "tested", Requires: []string{"coded"}},
		},
	}
	spec, unexpressed := Build(l)
	if len(unexpressed) != 0 {
		t.Fatalf("unexpected gaps: %v", unexpressed)
	}
	var names []string
	for _, st := range spec.States {
		names = append(names, st.Name)
	}
	want := []string{"backlog", "code", "test", "release"}
	if !equalStrings(names, want) {
		t.Errorf("derived order = %v, want %v", names, want)
	}
}

func TestBuildNamesUnsatisfiablePrerequisite(t *testing.T) {
	_, unexpressed := Build(List{Steps: []Step{
		{Name: "code", Agent: "executor", Provides: "coded", Requires: []string{"planned"}},
	}})
	if len(unexpressed) == 0 || !strings.Contains(unexpressed[0], "planned") {
		t.Fatalf("an obligation with no provider must be NAMED, got %v", unexpressed)
	}
}

func TestBuildNamesCycle(t *testing.T) {
	_, unexpressed := Build(List{Steps: []Step{
		{Name: "a", Agent: "executor", Provides: "x", Requires: []string{"y"}},
		{Name: "b", Agent: "executor", Provides: "y", Requires: []string{"x"}},
	}})
	if len(unexpressed) == 0 || !strings.Contains(unexpressed[0], "cycle") {
		t.Fatalf("a prerequisite cycle must be NAMED, got %v", unexpressed)
	}
}

// TestBuildSynthesisesTopology is the binary-owns-topology claim in miniature:
// the author declares neither cancel edges nor the park state, and both appear.
func TestBuildSynthesisesTopology(t *testing.T) {
	spec, _ := Build(List{
		Steps: []Step{
			{Name: "backlog", Start: true, Provides: "raised"},
			{Name: "code", Agent: "executor", Provides: "coded", Requires: []string{"raised"}},
			{Name: "done", Terminal: true, Requires: []string{"coded"}},
		},
		Cancel: "cancelled", CancelGate: "cancel-review",
		Park: "blocked", ParkGate: "blocked-review",
	})
	for _, want := range []string{"backlog->cancelled", "code->cancelled", "blocked->cancelled"} {
		if !containsEdge(spec, want) {
			t.Errorf("synthesised edge %s missing; edges = %v", want, edgeSet(spec))
		}
	}
	if containsEdge(spec, "done->cancelled") {
		t.Errorf("terminal state must not gain a cancel edge; edges = %v", edgeSet(spec))
	}
	if !spec.IsParkState("blocked") {
		t.Errorf("blocked must be a park state")
	}
	if !spec.IsTerminalState("done") {
		t.Errorf("done must be terminal")
	}
	if problems := wfdot.Validate(spec); len(problems) != 0 {
		t.Errorf("synthesised spec must validate, got %v", problems)
	}
}

func containsEdge(s wfdot.Spec, want string) bool {
	for _, e := range edgeSet(s) {
		if e == want {
			return true
		}
	}
