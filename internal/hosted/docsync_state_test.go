...
func TestDocumentCursorRoundTrip(t *testing.T) {
	...
	got, err := LoadDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/a")
	if err != nil {
		t.Fatal(err)
	}
	if got != "" {
	...
	if err := SaveDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/a", "cur-1"); err != nil {
		t.Fatal(err)
	}
	got, err = LoadDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/a")
	...
	got, err = LoadDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/b")
	if err != nil || got != "" {
		t.Fatalf("other repo = %q, %v; want empty", got, err)
	}
	if err := SaveDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/b", "cur-b"); err != nil {
		t.Fatal(err)
	}
	// First key still intact.
	got, _ = LoadDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/a")
	if got != "cur-1" {
		t.Errorf("repo a clobbered: %q", got)
	}
	got, _ = LoadDocumentCursor("https://s.example", "ws1", "proj-a", "/repo/b")
	if got != "cur-b" {
		t.Errorf("repo b = %q, want cur-b", got)
	}
	// Different project does not collide on the same repoRoot.
	got, err = LoadDocumentCursor("https://s.example", "ws1", "proj-b", "/repo/a")
	if err != nil || got != "" {
