...
description: Exit gate for a task EXECUTION (in_progress → done). An isolated, read-only reviewer judges whether the run may close — that the task's ACTION was actually carried out and its VERIFICATION is satisfied, reading the repo for evidence. Reviewers-only (judges, never enacts). The gate into a task execution's terminal done (satelle-done-is-last). Repo skill for the satelle executable-task machinery (sty_a25337ae); pushes back with specifics.
---
